...
- 新增 **Agnes 2.5 全能视频**节点（免费版与付费版）：使用 Agnes-Video-2.5-Flash / Agnes-Video-2.5 模型，集成**文生视频、图生视频、首尾帧、全能参考**四种模式，通过「模式」参数自动切换输入端口与参数；付费版额外支持 720P/960P/2K 分辨率，全能参考模式还支持传入参考视频。视频生成结果以帧序列（IMAGE）、音频（AUDIO）、帧率（FLOAT）三个端口输出。
- **Agnes 图像理解对话**节点增强：图像输入端口自动衍生，最多可同时传入 8 张图片进行视觉问答、图像描述等。
